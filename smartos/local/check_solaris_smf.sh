...
checkOptions() {
    ...
    OPTIONS="-w MNT -c DGD,OFF"

    while getopts ef:o:w:c:lhvV OPT $OPTIONS; do
            case $OPT in
                e) ERRORS_ONLY=1
                   ;;
...
if [ $CRITICAL -eq 1 ]; then
    echo "$STATE_CRITICAL SMF_SVCS - CRITICAL $STATE_TEXT"
fi
if [ $WARNING -eq 1 ]; then
    echo "$STATE_WARNING SMF_SVCS - WARNING $STATE_TEXT"
fi

echo "$STATE_OK SMF_SVCS - OK $STATE_TEXT"
